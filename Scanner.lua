-- FORENSE ANALYZER V7 - ANTI-CRASH TOTAL
local ok, err = pcall(function()
    ...
    -- Inyeccion universal (evita fallos de CoreGui strictos)
    ...
    for _, v in pairs(parentUI:GetChildren()) do
        if v.Name == "ForenseV7" then v:Destroy() end
    end
    
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "ForenseV7"
    ...
    MainFrame.BorderColor3 = Color3.fromRGB(200, 200, 0)
    ...
    TitleArea.Text = "  FORENSE ANALYZER V7 - MODO SEGURO"
    ...
    ActionBtn.Text = "/// CLICK AQUI PARA INICIAR EL ANALISIS ///"
    ...
    LogWindow.Text = "\n  SI VES ESTO, LA GUI CARGO PERFECTAMENTE.\n\n  Haz click en el boton rojo de arriba para scanear un mob."
    ...
    ActionBtn.MouseButton1Click:Connect(function()
                ...
                if captOn and (m=="fireserver" or m=="invokeserver") then
                    pcall(function()
                        ...
                        if not string.find(nL,"mouse") and not string.find(nL,"camera") then
                            table.insert(pkts,{t=tick()-t0, name=nm, path=s2.Name, cls=s2.ClassName, args={...}, rem=s2})
                        end
                    end)
                end
                return cHk(s2,...)
        ...
        local ToolRF = nil
        pcall(function() ToolRF = game.ReplicatedStorage.Shared.Packages.Knit.Services.ToolService.RF.ToolActivated end)
        ...
        for rName,rp in pairs(grps) do
            log = log .. " " .. rName .. " -> llamadox" .. #rp .. " veces (" .. rp[1].cls .. ")\n"
        end
...
if not ok then
    warn("============= ERROR FATAL EN FORENSE ANALYZER =============")
    warn(err)
    if getgenv then
        getgenv().LATEST_FORENSE_ERROR = err
    end
end
