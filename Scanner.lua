...
local LOCAL_MIRROR_ENABLED_ITEMS = {
    ["Flashlight"] = true,
    ["Blacklight"] = true
}

local function IsToolPowered(item)
    return item and (item:GetAttribute("Enabled") == true or item:GetAttribute("Power") == true)
end

local function IsFloorToggleableItem(realItemName)
    return FLOOR_TOGGLEABLE_ITEMS[realItemName] == true
end

local function FindWorldToolInstance(realItemName, anchorPos)
    ...
    for _, candidate in ipairs(itemsFolder:GetChildren()) do
        ...
        if candidateName == realItemName then
            ...
            local candidatePart = candidate.PrimaryPart or candidate:FindFirstChildWhichIsA("BasePart", true)
            if candidatePart then
                local dist = (candidatePart.Position - anchorPos).Magnitude
...
local function ResolveObservableToolInstance(itemRef, realItemName, anchorPos)
    local itemsFolder = Workspace:FindFirstChild("Items")
    if itemRef and itemRef.Parent == itemsFolder then
        return itemRef
    end
    local worldItem = FindWorldToolInstance(realItemName, anchorPos)
    if worldItem then
        return worldItem
    end
...
local function EnsureDroppedToolEnabled(remToggle, realItemName, anchorPos)
    if not remToggle or not IsFloorToggleableItem(realItemName) then
        return nil, false
    end

    local observed = nil
    for _ = 1, 20 do
        observed = FindWorldToolInstance(realItemName, anchorPos)
        if observed then
            break
        end
        task.wait(0.1)
    ...
    if not observed then
        AddLog("       ⚠️ No encontré la copia del piso para " .. realItemName .. ".", Color3.fromRGB(255, 180, 100))
        return nil, false
    end

    if IsToolPowered(observed) then
        AddLog("       ✅ " .. realItemName .. " quedó encendida en el piso.", Color3.fromRGB(0, 255, 120))
        return observed, true
    end

    for attempt = 1, 2 do
        remToggle:FireServer(observed)
        AddLog("       🔁 Re-encendiendo en el piso: " .. realItemName .. " (intento " .. attempt .. ")", Color3.fromRGB(255, 220, 120))
        for _ = 1, 6 do
            task.wait(0.1)
            if IsToolPowered(observed) then
                AddLog("       ✅ " .. realItemName .. " confirmó encendido después de soltarse.", Color3.fromRGB(0, 255, 120))
                return observed, true
            end
        end
    end

    AddLog("       ⚠️ " .. realItemName .. " quedó plantada, pero el piso no confirmó su estado.", Color3.fromRGB(255, 180, 100))
    return observed, false
end

local function MirrorHeldToggleState(item, realItemName, desiredEnabled)
    if item and LOCAL_MIRROR_ENABLED_ITEMS[realItemName] then
        item:SetAttribute("Enabled", desiredEnabled)
    end
end

local function ReportPlacedToolState(realItemName, itemNameLower, itemRef, anchorPos)
    local observed = ResolveObservableToolInstance(itemRef, realItemName, anchorPos)
    ...
    if pingActivo then
        ...
        task.spawn(function()
            while pingActivo do
                ...
                local tomables = {}
                local itemsFolder = Workspace:FindFirstChild("Items")
                local remToggleFloor = game.ReplicatedStorage:FindFirstChild("Events") and game.ReplicatedStorage.Events:FindFirstChild("ToggleItemState")
                for _, obj in ipairs(todasHerramientas) do
                    ...
                    if not obj:IsDescendantOf(game.Players) and (not obj.Parent or not obj.Parent:FindFirstChild("Humanoid")) then
                        ...
                        if not AutoLabShouldIgnoreItemName(n) then
                            ...
                            if isPlanted and obj.Parent == itemsFolder then
                                local realWorldName = tostring(obj:GetAttribute("ItemName") or obj:GetAttribute("DisplayName") or obj.Name)
                                local hasStateAttr = obj:GetAttribute("Enabled") ~= nil or obj:GetAttribute("Power") ~= nil
                                if remToggleFloor and IsFloorToggleableItem(realWorldName) and hasStateAttr and not IsToolPowered(obj) then
                                    pcall(function()
                                        remToggleFloor:FireServer(obj)
                                    end)
                                    AddLog("🔁 Reactivando herramienta plantada: " .. realWorldName, Color3.fromRGB(255, 220, 120))
                                    TraceTarget("ITEM_STATE", "RetoggledPlacedTool", {
                                        itemName = realWorldName,
                                        parent = obj.Parent
                                    })
                                    task.wait(0.15)
                                end
                    ...
                    if filledSlot and capturedItemName then
                        ...
                        if materializado then
                            ...
                            if itemFalso then
                                ...
                                local dropAnchorPos = ghostPart and ghostPart.Position or (LP.Character and LP.Character.PrimaryPart and LP.Character.PrimaryPart.Position) or nil
                                local observedWorldTool = nil
                                ...
                                pcall(function()
                                    ...
                                    if string.find(itemNameLower, "video camera") then
                                        pcall(function()
                                            ...
                                            if useItemEvent then
                                                useItemEvent:Fire()
                                                AddLog("       🔌 Orden de activación enviada por UseItem", Color3.fromRGB(0, 255, 200))
                                            end
                                            if typeof(remToggle) == "Instance" then 
                                                remToggle:FireServer(itemFalso) 
                                                AddLog("       🔌 Toggle especial enviado al host para la cámara", Color3.fromRGB(200, 200, 100))
                                            end
                                        end)
                                        task.wait(0.8)
                                        ...
                                        skipDrop = true
                                        observedWorldTool = ResolveObservableToolInstance(nil, realItemName, dropAnchorPos)
                                        AddLog("       📹 " .. realItemName .. " plantada con sistema de trípode.", Color3.fromRGB(0, 255, 150))
                                    ...
                                    elseif string.find(itemNameLower, "laser") then
                                        pcall(function()
                                            ...
                                            if useItemEvent then
                                                useItemEvent:Fire()
                                                AddLog("       🔌 Orden de encendido enviada por UseItem", Color3.fromRGB(0, 255, 200))
                                            elseif typeof(remToggle) == "Instance" then
                                                remToggle:FireServer(itemFalso)
                                                AddLog("       🔌 Toggle enviado al Laser Projector (fallback)", Color3.fromRGB(200, 200, 100))
                                            end
                                        end)
                                        task.wait(0.8)
                                        ...
                                        skipDrop = true
                                        observedWorldTool = ResolveObservableToolInstance(nil, realItemName, dropAnchorPos)
                                        AddLog("       📍 " .. realItemName .. " soltado en el cuarto; verificando estado real en el piso...", Color3.fromRGB(0, 255, 150))
                                    ...
                                    elseif string.find(itemNameLower, "thermometer") then
                                        pcall(function()
                                            ...
                                            if useItemEvent then
                                                useItemEvent:Fire()
                                                AddLog("       🌡️ Orden de encendido enviada al termómetro por UseItem", Color3.fromRGB(200, 200, 100))
                                            else
                                                if typeof(remToggle) == "Instance" then remToggle:FireServer(itemFalso) end
                                                AddLog("       🌡️ Toggle enviado al termómetro (fallback)", Color3.fromRGB(200, 200, 100))
                                            end
                                        end)
                                    ...
                                    elseif string.find(itemNameLower, "flashlight") and not string.find(itemNameLower, "blacklight") then
                                        local yaEncendida = IsToolPowered(itemFalso)
                                        if not yaEncendida and remToggle then
                                            MirrorHeldToggleState(itemFalso, realItemName, true)
                                            remToggle:FireServer(itemFalso)
                                            AddLog("       🔦 Orden de encendido enviada a Flashlight", Color3.fromRGB(100, 255, 100))
                                        elseif yaEncendida then
                                            AddLog("       ✅ Flashlight ya estaba encendida en mano.", Color3.fromRGB(150, 255, 150))
                                        end
                                        
                                    elseif string.find(itemNameLower, "blacklight") or string.find(itemNameLower, "uv light") then
                                        if not IsToolPowered(itemFalso) and remToggle then
                                            MirrorHeldToggleState(itemFalso, realItemName, true)
                                            remToggle:FireServer(itemFalso)
                                            AddLog("       🟣 Orden de encendido enviada a Blacklight", Color3.fromRGB(100, 255, 100))
                                        end
                                        task.wait(0.3)
                                        pcall(function()
                                    ...
                                    elseif string.find(itemNameLower, "emf") then
                                        local yaEncendido = IsToolPowered(itemFalso)
                                        if not yaEncendido and remToggle then 
                                            remToggle:FireServer(itemFalso)
                                            AddLog("       📟 Orden de encendido enviada a EMF Reader", Color3.fromRGB(100, 255, 100))
                                        elseif yaEncendido then
                                            AddLog("       ✅ EMF Reader ya estaba encendido en mano.", Color3.fromRGB(150, 255, 150))
                                        end
                                    
                                    elseif string.find(itemNameLower, "spirit box") then
                                        local yaEncendida = IsToolPowered(itemFalso)
                                        if not yaEncendida and remToggle then 
                                            remToggle:FireServer(itemFalso)
                                            AddLog("       📻 Orden de encendido enviada a Spirit Box", Color3.fromRGB(100, 255, 100))
                                        elseif yaEncendida then
                                            AddLog("       ✅ Spirit Box ya estaba encendida en mano.", Color3.fromRGB(150, 255, 150))
                                        end
                                    ...
                                    elseif string.find(itemNameLower, "music box") then
                                        ...
                                        if not yaEncendido and remToggle then 
                                            remToggle:FireServer(itemFalso)
                                            AddLog("       🔋 Orden de encendido enviada a " .. tostring(realItemName), Color3.fromRGB(100, 255, 100))
                                        elseif yaEncendido then
                                            AddLog("       ✅ " .. tostring(realItemName) .. " ya venía encendida en mano.", Color3.fromRGB(150, 255, 150))
                                        end
                                    end
                                ...
                                if not observedWorldTool then
                                    observedWorldTool = ResolveObservableToolInstance(nil, realItemName, dropAnchorPos)
                                end
                                
                                if IsFloorToggleableItem(realItemName) then
                                    local ensuredTool, confirmedOn = EnsureDroppedToolEnabled(remToggle, realItemName, dropAnchorPos)
                                    if ensuredTool then
                                        observedWorldTool = ensuredTool
                                    end
                                    if not confirmedOn and observedWorldTool then
                                        TraceTarget("ITEM_STATE", "DroppedToolUnconfirmed", {
                                            itemName = realItemName,
                                            enabled = observedWorldTool:GetAttribute("Enabled"),
                                            power = observedWorldTool:GetAttribute("Power"),
                                            parent = observedWorldTool.Parent
                                        })
                                    end
                                end
                                ...
                                pcall(function()
                                    local auditItem = observedWorldTool or itemFalso
                                    if auditItem then
                                        ReportPlacedToolState(realItemName, itemNameLower, auditItem, dropAnchorPos)
                                    end
                                end)
