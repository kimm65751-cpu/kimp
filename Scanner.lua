...
local function traceScriptEnv(scriptInst, label)
    if not scriptInst or not getsenv then
        return
    end
    local ok, env = pcall(getsenv, scriptInst)
    if not ok or type(env) ~= "table" then
        writeLine("RUNTIME", "ScriptEnvMiss", {
            script = label,
...
local function scanRuntimeCandidates(force)
    ...
    for _, scriptName in ipairs({
        ...
        if found then
            ...
            writeLine("RUNTIME", "ScriptMissing", { script = scriptName }, Color3.fromRGB(255, 180, 120))
        end
    end

    if not getgc then
        writeLine("RUNTIME", "GetGCUnavailable", nil, Color3.fromRGB(255, 180, 120))
        return
    end
    ...
    if not ok or type(gcList) ~= "table" then
        writeLine("RUNTIME", "GetGCError", nil, Color3.fromRGB(255, 180, 120))
        return
    end
    ...
    writeLine("RUNTIME", "ScanSummary", {
        scanned = scanned,
        newCandidates = foundCount
