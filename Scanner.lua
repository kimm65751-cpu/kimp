...
local function buildGui()
    ...
    local scanButton = makeButton(title, "SCAN", -240, 64)
    local clearButton = makeButton(title, "CLEAR", -310, 64)
...
local function classifyRoot(root)
    ...
    local rewardText = lower(attrs.RewardRes or attrs.Reward)

    if string.find(path, "areapickup", 1, true) or attrs.RewardRes ~= nil or attrs.Reward ~= nil then
        if rewardText == "egg" or string.find(name, "egg", 1, true) or string.find(name, "huevo", 1, true) then
            return "Egg", attrs
        end
        return "Pickup", attrs
    end
...
local function formatPickupLabel(kind, root, attrs)
    local label = kind
    if attrs.RewardRes then
        label = label .. " | " .. tostring(attrs.RewardRes)
    else
        label = label .. " | " .. tostring(root.Name)
    end
    if attrs.TmplId then
        label = label .. " #" .. tostring(attrs.TmplId)
    end
    return label
end
...
local function snapshotWorld()
    ...
    if clientMonsters then
        for _, child in ipairs(clientMonsters:GetChildren()) do
            monsterCount = monsterCount + 1
            local adornee = findAdornee(child)
            if rootPart and adornee and #nearest < 4 then
                local _, attrs = classifyRoot(child)
                local battle, caught = getBattleSummary(attrs)
                table.insert(nearest, {
                    id = attrs.MonsterId,
                    level = attrs.Level,
                    dist = math.floor((adornee.Position - rootPart.Position).Magnitude),
                    battle = battle,
                    caught = caught
                })
            end
    ...
    if clientPets then
        for _, child in ipairs(clientPets:GetChildren()) do
            petCount = petCount + 1
        end
    end
    ...
    hookRemoteEvent(events:FindFirstChild("Message"), function(...)
        ...
        if IMPORTANT_MESSAGE_KINDS[kind] then
            if kind == "PetHealthSync" then
                ...
                local newHealth = tonumber(args[3])
