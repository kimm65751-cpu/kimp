    ...
    if posibles == 1 and not _G.MatchCompletado then
        ...
        coroutine.wrap(function()
            ...
            if rsEvents and rsEvents:FindFirstChild("EvidenceMarkedInJournal") then
                ...
                pcall(function()
                    local LP = game:GetService("Players").LocalPlayer
                    local appJournal = LP.PlayerGui:FindFirstChild("AppJournal", true)
                    if appJournal then
                        -- El juego usa 'Page4' -> 'Right' -> 'Page' -> 'GhostTypes' para la SELECCIÓN REAL.
                        -- Page1 solo sirve para navegar en la wiki local.
                        local page4 = appJournal:FindFirstChild("Page4", true)
                        if page4 then
                            local rightSide = page4:FindFirstChild("Right")
                            local realGhostTypes = rightSide and rightSide:FindFirstChild("Page") and rightSide.Page:FindFirstChild("GhostTypes")
                            
                            if realGhostTypes and realGhostTypes:FindFirstChild(networkName) then
                                local btn = realGhostTypes[networkName]:FindFirstChild("Detection", true)
                                if btn and getconnections then
                                    local conns = getconnections(btn.MouseButton1Click)
                                    if conns and conns[1] then
                                        conns[1]:Fire() -- UN solo clic: nil→true
                                        AddLog("       🖱️ Clic UI enviado a Page4.Right...GhostTypes[" .. networkName .. "]", Color3.fromRGB(200, 200, 100))
                                    end
                                end
                            end
                        end
    ...
    if pingActivo then
        ...
        task.spawn(function()
            while pingActivo do
                ...
                -- 🚀 V8.50: FILTRO ESTRICTO Y BÚSQUEDA GLOBAL PROFUNDA (Caza de Ghost y Orb en 1 solo ciclo)
                local ghostPos = nil
                local currentGhostInstance = nil
                local realOrbInstance = nil
                ...
                for _, obj in pairs(workspace:GetDescendants()) do
                    ...
                    -- Buscar el Orb (incluso si los desarrolladores lo escondieron en una sub-carpeta)
                    if obj.Name == "GhostOrb" or string.lower(obj.Name) == "ghostorb" then
                        realOrbInstance = obj
                    end
                end
                ...
                -- V8.78: Muchos mapas cargan un Orbe oculto debajo del mapa (Dullahan).
                local orb = realOrbInstance or workspace:FindFirstChild("GhostOrb")
                if orb and not EvidenciasEncontradas["Orbe Fantasma"] then
                    ...
                    if ghostPos and (orbPos - ghostPos).Magnitude <= 35 then
                        ...
                        -- V8.91 - PARCHE CIBERNÉTICO ANTI-SKINWALKER (Del archivo .txt):
                        -- La clase Skinwalker tiene la bandera 'FakeGhostOrb = true' para trollear.
                        local isFakeOrbFlag = currentGhostInstance and currentGhostInstance:GetAttribute("FakeGhostOrb")
                        if isFakeOrbFlag == true then
                            isRealOrb = false
                            AddLog("🛡️ FIREWALL: Orbe Fantasma ignorado (El motor confesó 'FakeGhostOrb' en el ADN de la entidad).", Color3.fromRGB(255, 100, 100))
                        -- Parche Profundidad Relativa
                        elseif (ghostPos.Y - orbPos.Y) > 10 then 
                            isRealOrb = false 
                        else
                            -- V8.84: Muchos orbes existen físicamente pero apagados (El motor apaga las partículas).
                            for _, particle in pairs(orb:GetDescendants()) do
                                if particle:IsA("ParticleEmitter") and particle.Enabled == true and particle.Rate > 0 then
                                    isRealOrb = true
                                    break
                                end
                            end
                            
                            -- Respaldo visual si no usa partículas.
        ...
        task.spawn(function()
            while ScanEvi do
                for _, obj in pairs(Workspace:GetDescendants()) do
                    ...
                    -- (Detección de Orbe Fantasma eliminada de aquí para evitar overrides de falsos o objetos enterrados.
                    --  Se maneja exclusivamente con la rutina principal segura GhostOrb del auto-lab)
                        ...
                        local function InyectarBox(box)
                            task.spawn(function()
                                pcall(function()
                                    ...
                                    if t then
                                        -- Detectar cambio de Audio (Cuando el fantasma inyecta un AssetId nuevo)
                                        -- La estática base no cambia de ID, así que esto evita el falso positivo.
