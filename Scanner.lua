...
local function summarizeProbeValue(value)
    ...
    if valueKind == "Vector3" or valueKind == "CFrame" then
        return {
            kind = valueKind,
    ...
    if luaKind == "table" then
        return summarizeExportValue(value)
    end
    if luaKind == "function" then
    ...
    return {
        kind = luaKind,
        value = trimText(safeTostring(value), 140)
    }
end

local function summarizePackedResults(packed)
    local values = {}
    for index = 1, math.min(packed.n or 0, CONFIG.runtimeProbeMaxItems) do
        values[index] = summarizeProbeValue(packed[index])
    end
    return {
        count = packed.n or 0,
        values = values
...
local function summarizeIteratorResults(iterFn, state, seed)
    local items = {}
    local cursor = seed
    for _ = 1, CONFIG.runtimeProbeMaxItems do
        local ok, nextKey, nextValue = pcall(iterFn, state, cursor)
        if not ok then
            return {
                error = trimText(nextKey, 180),
                items = items
            }
        end
        if nextKey == nil then
        ...
        table.insert(items, {
            key = summarizeProbeValue(nextKey),
            value = summarizeProbeValue(nextValue)
        })
        cursor = nextKey
    end
    return {
...
local function logProbeCall(scriptName, probeName, args, packed)
    if packed.n >= 1 and type(packed[1]) == "function" then
        writeLine("MODULE", "ProbeIter", {
            script = scriptName,
            probe = probeName,
            args = args,
            iterator = summarizeIteratorResults(packed[1], packed[2], packed[3])
        }, Color3.fromRGB(255, 230, 150))
        return
    end

    writeLine("MODULE", "ProbeCall", {
        script = scriptName,
        probe = probeName,
        args = args,
        result = summarizePackedResults(packed)
    }, Color3.fromRGB(255, 230, 150))
end

local function logProbeError(scriptName, probeName, args, err)
    writeLine("MODULE", "ProbeError", {
        script = scriptName,
        probe = probeName,
        args = args,
        error = trimText(err, 180)
    }, Color3.fromRGB(255, 180, 120))
end

local function collectOwnedPetSampleIds()
    local sampleIds = {}
    local seen = {}
    local pets = Workspace:FindFirstChild("Pets")
    local myId = tostring(LP and LP.UserId or "")
    if not pets then
        return sampleIds
    end
    ...
    for _, pet in ipairs(pets:GetChildren()) do
        ...
        local petItemId = pet:GetAttribute("PetItemId")
        if petItemId ~= nil and (playerId == myId or ownerUserId == myId) then
            local signature = tostring(petItemId)
            if not seen[signature] then
                seen[signature] = true
                table.insert(sampleIds, petItemId)
                if #sampleIds >= CONFIG.runtimeProbeMaxItems then
                    break
                end
            end
        end
...
local function collectMonsterProbeContext()
    local ctx = {
        ids = {},
        nearestId = nil,
        nearestPart = nil
    ...
    for _, monster in ipairs(monsters:GetChildren()) do
        ...
        if monsterId == nil then
            monsterId = tostring(monster.Name):match("^Monster_(%d+)$")
            if monsterId ~= nil then
                monsterId = tonumber(monsterId)
            end
        end
        if monsterId ~= nil then
            local signature = tostring(monsterId)
            if not seen[signature] then
                seen[signature] = true
                table.insert(ctx.ids, monsterId)
                if #ctx.ids >= CONFIG.runtimeProbeMaxItems then
                    -- keep collecting nearest part below even if ids are full
                end
            end
        end
        ...
        if rootPart and part then
            ...
            if dist < nearestDist then
                nearestDist = dist
                ctx.nearestPart = part
                ctx.nearestId = monsterId
            end
        elseif not ctx.nearestPart then
            ctx.nearestPart = part
            ctx.nearestId = monsterId
        end
    end
...
local function tryProbe(scriptName, probeName, fn, ...)
    if type(fn) ~= "function" then
        return
    end
    local args = { ... }
    local ok, packedOrErr = callProbe(fn, ...)
    if ok then
        logProbeCall(scriptName, probeName, args, packedOrErr)
    else
        logProbeError(scriptName, probeName, args, packedOrErr)
    end
end

local function runReadOnlyModuleProbes(scriptName, exported)
    if type(exported) ~= "table" then
        return
    end

    if scriptName == "MgrPetClient" then
        tryProbe(scriptName, "GetSelfPetInfo", exported.GetSelfPetInfo)
        tryProbe(scriptName, "IterPet", exported.IterPet)

        local petIds = collectOwnedPetSampleIds()
        if #petIds > 0 then
            tryProbe(scriptName, "GetPetInfo", exported.GetPetInfo, petIds[1])
            tryProbe(scriptName, "GetPetBoundingBox", exported.GetPetBoundingBox, petIds[1])
        end
        return
    end

    if scriptName == "MgrMonsterClient" then
        tryProbe(scriptName, "GetBattleBossMonsterInfo", exported.GetBattleBossMonsterInfo)
        tryProbe(scriptName, "IterMonster", exported.IterMonster)
        tryProbe(scriptName, "IterSelfBattleMonster", exported.IterSelfBattleMonster)
        tryProbe(scriptName, "IterSelfCanCatchBattleMonster", exported.IterSelfCanCatchBattleMonster)

        local ctx = collectMonsterProbeContext()
        if ctx.nearestId ~= nil then
            tryProbe(scriptName, "GetMonsterInfo", exported.GetMonsterInfo, ctx.nearestId)
            tryProbe(scriptName, "GetMonsterBoundingBox", exported.GetMonsterBoundingBox, ctx.nearestId)
        end
        if ctx.nearestPart ~= nil then
            tryProbe(scriptName, "GetMonsterIdByPart", exported.GetMonsterIdByPart, ctx.nearestPart)
        end
        return
    end
end
            ...
            if found:IsA("ModuleScript") then
                if loadedSet[found] then
                    ...
                    if okRequire then
                        ...
                        runReadOnlyModuleProbes(scriptName, exported)
                    else
