...
task.spawn(function()
    while true do
        ...
        -- PASO 1: Click el mob para INICIAR/MANTENER combate
        -- Sin este step, TryUseSkill no tiene target activo y lo ignora
        if targetCd then
            pcall(function() fireclickdetector(targetCd) end)
        end

        -- PASO 2: Spammear TryUseSkill 10x ahora que hay combate activo
        ...
        -- Reporte
        if oks > 0 then
            Log("💥 Click+TryUseSkill "..oks.."/10 → "..targetMob.Name.." ["..math.floor(targetDist).."m]", Color3.fromRGB(100, 255, 100))
        else
            Log("❌ TryUseSkill falló todas. Err: "..tostring(lastErr), Color3.fromRGB(255, 80, 80))
        end
    end
end)
