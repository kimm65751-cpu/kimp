    ...
    ExploitBtn.Text = "[X] 7. EXPLOIT FORENSE TOTAL (MOBS + RED)"
    ...
    -- =====================================================
    -- ANALIZADOR FORENSE TOTAL - 17 VECTORES DE COMBATE + RED
    -- =====================================================
    ExploitBtn.MouseButton1Click:Connect(function()
        ExploitBtn.Text = "[...] ANALIZANDO (~5s)..."
        ...
        if not mob then
            AddLog("[!]", "Sin mob vivo cerca", "Acercate a un zombi primero y reintenta.")
            ExploitBtn.Text = "[X] 7. EXPLOIT FORENSE TOTAL (MOBS + RED)"
            ExploitBtn.BackgroundColor3 = Color3.fromRGB(160, 10, 10)
            return
        end

        AddLog("[MOB]", mob.Name.." HP:"..math.floor(mobHum.Health).."/"..math.floor(mobHum.MaxHealth).." Dist:"..math.floor(closestD).."m", "Iniciando 17 vectores de analisis forense...")
        ...
        v2 = dsc>0 and "Scripts dentro del mob ("..dsc.."):\n"..v2.."Oportunidad: script:Disable()" or "Sin scripts locales. Dano via Raycast server-side."
        AddLog("V2", "Scripts de dano dentro del mob", v2); task.wait(0.05)

        -- V3: Validacion de direccion
        local v3 = myRoot and (
            "Dot (frente=1, espalda=-1): "..string.format("%.2f",(mobRoot.Position-myRoot.Position).Unit:Dot(myRoot.CFrame.LookVector)).."\nSi mob baja HP atacando desde atras = NO valida direccion."
        ) or "Sin HRP local."
        AddLog("V3", "Validacion de direccion para hacer dano", v3); task.wait(0.05)
        ...
        if myRoot then
            ...
            v5 = "Rotar mob de espaldas: "..(okR and "EXITOSO - agrega en Farm: mobRoot.CFrame=CFrame.lookAt(pos,awayPos)" or "Bloqueado por servidor.")
        end
        ...
        for _,part in pairs(mob:GetDescendants()) do
            if part:IsA("BasePart") and part:FindFirstChildWhichIsA("TouchTransmitter") then
                ...
                v6=v6..part.Name.." Size:"..tostring(part.Size).."\n  EXPLOIT: tt:Destroy() -> mob no detecta tu cuerpo\n"
            end
        end
        AddLog("V6", "TouchInterest y Dano Contacto", v6=="" and "Sin TouchInterest. Mob usa Raycast server-side." or v6); task.wait(0.05)
        ...
        for _,part in pairs(mob:GetDescendants()) do
            if part:IsA("BasePart") then
                ...
                if string.find(n,"arm") or string.find(n,"hand") or string.find(n,"weapon") or string.find(n,"hit") or string.find(n,"attack") then
                    ...
                    local ok=pcall(function() part.Size=Vector3.new(0.1,0.1,0.1) end)
                    v7=v7..part.Name.." reducido: "..(ok and "EXITOSO" or "Bloqueado").."\n"
                end
            end
        end
        AddLog("V7", "Brazos / Hitbox de Ataque", v7=="" and "Sin partes nombradas. Raycast ~5-8 studs. Solucion: muro a -6.5 studs." or v7); task.wait(0.05)
        ...
        AddLog("V8", "Congelar IA del Mob", "WalkSpeed=0: "..(okW and "EXITOSO" or "Bloqueado").."\nChangeState Disabled: "..(okS and "EJECUTADO" or "Bloqueado")); task.wait(0.05)

        -- V9: Flags invulnerabilidad
        local v9, fc = "", 0
        for _,c in pairs(mob:GetDescendants()) do
            if c:IsA("BoolValue") or c:IsA("NumberValue") or c:IsA("IntValue") or c:IsA("StringValue") then
                ...
                v9=v9..(sus and "[SOSPECHHOSO] " or "  ")..c.Name.." = "..tostring(c.Value).."\n"; fc=fc+1
            end
        end
        AddLog("V9", "Flags Invulnerabilidad", fc==0 and "Sin Values en mob. Flags son server-side." or v9); task.wait(0.05)

        -- V10: Recomendaciones
        local v10="Mob: "..mob.Name.."\n"
        v10=v10..(#touchParts>0 and "[1] TouchInterest: destruir tt en loop\n" or "")
        v10=v10..(#attackParts>0 and "[2] Brazos: reducir size a 0.1x en loop\n" or "")
        v10=v10.."[3] Muro: offset -6.5 studs\n[4] Si V5 OK: rotar mob en Farm\n"
        ...
        AddLog("V11", "Remotes de Combate", "Total: "..rcnt.."\n"..(rcnt==0 and "Sin nombres obvios. Usa Interceptor." or v11))

        -- V12: Captura en vivo
        AddLog("V12", "[RED] CAPTURA EN VIVO 3s", "Atacando mob y capturando C->S. Espera...")
        task.wait(0.3)
        ...
        -- V13: Curva HP
        local hpA=mobHum.Health; local hpDrop=hpB-hpA
        ...
        v13=v13..(hpDrop<=0 and "DANO NULO. ToolRF no valido aqui." or "ToolRF confirmado. "..string.format("%.1f",hpDrop).." HP quitados.")
        ...
        if #pkts==0 then v14=v14.."CERO paquetes. Usa Interceptor (btn 2) + ataque manual." end
        AddLog("V14", "Paquetes C->S (lo que envias al server)", v14)

        -- V15: S->C
        AddLog("V15", "Paquetes S->C", "HP sincronizado via Humanoid.Health replication (sin RemoteEvents).\nUsa Live Scan apuntando al mob para ver HP en tiempo real.")
